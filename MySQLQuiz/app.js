...
(() => {
    ...
    function highlightSQL(code) {
        let html = code
            ...
            .replace(/\b(SELECT|FROM|WHERE|INSERT|INTO|VALUES|UPDATE|SET|DELETE|CREATE|TABLE|DROP|ALTER|ADD|COLUMN|PRIMARY KEY|FOREIGN KEY|JOIN|INNER|LEFT|RIGHT|FULL|OUTER|ON|GROUP BY|ORDER BY|HAVING|ASC|DESC|LIMIT|OFFSET|AS|AND|OR|NOT|IN|LIKE|IS NULL|IS NOT NULL|COUNT|SUM|AVG|MIN|MAX|DISTINCT|CASE|WHEN|THEN|ELSE|END|UNION|ALL|EXISTS|BETWEEN)\b/gi, '<span class="kw">$1</span>')
            // Numbers
    ...
    function renderQuestion() {
        ...
        // Type badge
        if (q.type === 'code') {
            qTypeBadge.innerHTML = `<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="16 18 22 12 16 6"/><polyline points="8 6 2 12 8 18"/></svg> Query Analysis`;
        ...
        if (q.code) {
            ...
            codeLabel.textContent = q.type === 'code' ? 'Analyze this query' : 'Reference Information';
            codeContent.innerHTML = highlightSQL(escapeHTML(q.code));
        } else {
    ...
    function showResults() {
        ...
        // Title
        if (pct === 100) { resTitle.textContent = '🏆 Perfect Score!'; resSub.textContent = 'Database master! Flawless run.'; }
        else if (pct >= 85) { resTitle.textContent = '🌟 Outstanding!'; resSub.textContent = 'You really know your MySQL queries!'; }
        ...
        else if (pct >= 30) { resTitle.textContent = '📖 Keep Learning!'; resSub.textContent = 'Practice your joins and aggregations.'; }
    ...
    function buildCategoryBreakdown() {
        ...
        const colors = {
            'Basic Queries': '#E48E00',
            'Filtering & Sorting': '#fb923c',
            'Joins': '#00758F',
            'Aggregation': '#22d3ee',
            'Schema & Constraints': '#a78bfa',
            'Advanced': '#f472b6'
        };
