...
(() => {
    ...
    function highlightPHP(code) {
        let html = code
            ...
            // Keywords
            .replace(/\b(echo|return|function|if|else|elseif|switch|case|break|default|for|foreach|while|do|as|new|class|extends|implements|interface|abstract|public|private|protected|static|const|var|try|catch|finally|throw|require|include|require_once|include_once|use|namespace|array|isset|unset|empty|global|instanceof)\b/g, '<span class="kw">$1</span>')
            // Booleans & null
            .replace(/\b(true|false|TRUE|FALSE)\b/g, '<span class="bool">$1</span>')
            .replace(/\b(null|NULL)\b/g, '<span class="null">$1</span>')
            // Variables
    ...
    function renderQuestion() {
        ...
        if (q.code) {
            ...
            codeContent.innerHTML = highlightPHP(escapeHTML(q.code));
        } else {
    ...
    function showResults() {
        ...
        if (pct === 100) { resTitle.textContent = '🏆 Perfect Score!'; resSub.textContent = 'PHP master — flawless execution!'; }
        else if (pct >= 85) { resTitle.textContent = '🌟 Outstanding!'; resSub.textContent = 'You really know your PHP!'; }
        else if (pct >= 70) { resTitle.textContent = '👏 Great Job!'; resSub.textContent = 'Strong fundamentals — keep it up!'; }
        else if (pct >= 50) { resTitle.textContent = '👍 Not Bad!'; resSub.textContent = 'Decent, but review OOP and arrays.'; }
        else if (pct >= 30) { resTitle.textContent = '📖 Keep Studying!'; resSub.textContent = 'Review scope, arrays, and string functions.'; }
        else { resTitle.textContent = '💪 Don\'t Give Up!'; resSub.textContent = 'Start with the basics and build up!'; }
